        ...
        return new Promise((resolve) => {    
            ...
            this.initDB().then((db) => {      
                db.transaction((tx) => {     
                    ...
                    tx.executeSql('SELECT n.id, n.title, n.content, n.archived, n.image, n.alarm FROM Notes n', []).then(([tx,results]) => {          
        ...
        return new Promise((resolve) => {    
            this.initDB().then((db) => {      
                db.transaction((tx) => {     
                    ...
                    tx.executeSql('INSERT INTO Notes VALUES (?, ?, ?, ?, ?, ?)', [note.id, note.title, note.content, note.archived, note.image, note.alarm]).then(([tx, results]) => { 
                        resolve(results);        
                    });      
                }).then((result) => {        
                    this.closeDatabase(db);      
